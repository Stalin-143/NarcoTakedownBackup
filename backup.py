...
def load_model():
    ...
    try:
        with open("narcotic_classifier_model.pkl", "rb") as f:
            models = pickle.load(f)
            logger.info(f"Available keys in model file: {list(models.keys())}")

            # Check for 'trained_models' dict
            if 'trained_models' in models:
                trained_models = models['trained_models']
                if 'svm' in trained_models:
                    classifier = trained_models['svm']
                    logger.info("Loaded SVM model from trained_models.")
                elif 'random_forest' in trained_models:
                    classifier = trained_models['random_forest']
                    logger.info("Loaded Random Forest model from trained_models.")
                else:
                    raise KeyError("No recognized classifier in 'trained_models'.")
            else:
                raise KeyError("Key 'trained_models' not found in pickle file.")

        tokenizer = AutoTokenizer.from_pretrained("distilbert-base-uncased")
        text_model = AutoModel.from_pretrained("distilbert-base-uncased")
        logger.info("Tokenizer and text model loaded successfully.")
